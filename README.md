...
Important advantages of scapegoat BSTs over other BSTs are simplicity and reduced memory consumption. XOR scapegoat BSTs introduce additional complexity with an associated performance hit, compared to plain scapegoat BSTs; insertion and erasure by key can perform worse than when using a plain scapegoat BST. This leaves tree traversal and erasure by node as the only advantage the XOR scapegoat BST has over the plain scapegoat BST.
